...
### 수정 전 상태 전이 메서드 (MediaEncodeJob.java)
...
| 상황 | 수정 전 동작 |
|---|---|
| DONE → DONE (중복 callback) | **예외 발생 → worker 재시도 → 무한반복** |
| FAILED → FAILED (중복 callback) | **예외 발생 → worker 재시도 → 무한반복** |

---

## 3. 수정 내용

terminal 상태(DONE, FAILED)에 이미 도달했으면 중복 callback을 조용히 무시(return)하도록 수정했어요.
...
### markProcessing()은 수정하지 않은 이유

PROCESSING은 terminal 상태가 아니에요. 중복 PROCESSING callback은 worker가 같은 job을 두 번 시작하는 것으로, 예외를 던져서 명확히 오류로 처리하는 게 맞아요.
...
| DONE → FAILED | 예외 발생 (진짜 이상한 상황) |
| PROCESSING → PROCESSING (중복) | 예외 발생 (진짜 이상한 상황) |
